...
class FetcherUrl extends Fetcher {
  ...
  function get_data($data_id) {
	...
	if ($this->fetch($data_id)) {
	  ...
	  return new FetchedDataURL($this->content, explode("\r\n",$this->headers), $this->url);
	  ...
	  $this->error_message .= ob_get_contents();
	  ob_end_clean();
	  error_log(sprintf("Cannot open %s, too many redirects", $data_id));
	  return null;
	} else {
	  $_server_response = $this->headers;
	  $_url = htmlspecialchars($data_id);
	  ...
	  $this->error_message .= ob_get_contents();
	  ob_end_clean();
	  #error_log(sprintf("Cannot openX %s", $data_id));
	  return null;
	}
  }
  ...
  function _connect() {
	...
	if (!$fp) {
	  $message = sprintf("Cannot connect to %s:%d - (%d) %s", $this->host, $this->port, $errno, $errstr);
	  error_log($message);
  ...
  function _connect_ssl() {
	...
	if (!extension_loaded('openssl')) {
	  $message = sprintf("Cannot connect to %s:%d. SSL Extension missing", $this->host, $this->port);
	  error_log($message);
	  $this->error_message .= $message;
	  return null;
	};
	...
	if (!$fp) {
	  $message = sprintf("Cannot connect to %s:%d - (%d) %s<br/>Missing SSL support?", $this->host, $this->port, $errno, $errstr);
	  error_log($message);
	  $this->error_message = $message;
	  return null;
	};
